...
class ObservableBiologicalAgent:
    ...
    def evaluate(self, products_list: List[Dict]) -> Dict:
        ...
        if "filter" in self.clients:
            # FIXED: Convert ingredient names to expected format
            ingredients_list = [{"name": ing} for ing in all_ingredients]
        ...
        # If filter server returned empty, fallback to simple classification
        if not chemicals and not non_chemicals and all_ingredients:
            # Simple fallback classification
            non_chemical_keywords = ["AQUA", "WATER", "SODIUM CHLORIDE", "SEED OIL", "JOJOBA", 
                                     "BUTTER", "SHEA", "F.I.L", "Z288697", "2", "OIL", "WAX"]
            for ing in all_ingredients:
                is_non = False
                for kw in non_chemical_keywords:
                    if kw.upper() in ing.upper():
                        is_non = True
                        break
                if is_non:
                    non_chemicals.append(ing)
                else:
